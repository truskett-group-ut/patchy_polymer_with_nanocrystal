...
import numpy as np
class KovalenkoHirata(AtomicClosure):
    ...
    **Description**
    ...
    def __repr__(self):
        return '<AtomicClosure: KovalenkoHirata>'
    ...
    def calculate(self,r,gamma):
        ...
        if self.apply_hard_core:
            ...
            # calculate closure outside hard core but in regions to use HNC form
            mask = (r>self.sigma)*((-self.potential+gamma)<=0)
            self.value[mask] = -gamma[mask]-1.0+np.exp(-self.potential[mask]+gamma[mask])
            
            mask = (r>self.sigma)*((-self.potential+gamma)>0)
            self.value[mask] = -self.potential[mask]
        else:
            self.value = -gamma-1.0+np.exp(-self.potential+gamma)
            
            mask = ((-self.potential+gamma)>0)
            self.value[mask] = -self.potential[mask]
...
class KH(KovalenkoHirata):
    '''Alias of KovalenkoHirata'''
    pass
